...
/* Busca una variable de entorno probando varios nombres, y si ninguno está,
   barre el entorno buscando cualquiera que acabe igual.
   Hace falta porque Vercel bautiza las variables de forma distinta según cómo
   crees el almacén: DATABASE_URL a secas, o STORAGE_DATABASE_URL con prefijo. */
function variable(nombres, sufijo, excluir) {
  for (let i = 0; i < nombres.length; i++) {
    if (process.env[nombres[i]]) return process.env[nombres[i]];
  }
  if (sufijo) {
    const claves = Object.keys(process.env).filter((k) => (
      k.endsWith(sufijo) && process.env[k] && !(excluir && excluir.test(k))
    )).sort();
    if (claves.length) return process.env[claves[0]];
  }
  return '';
}
...
module.exports = {
  ...
  json, leerCuerpo, nombreSeguro, variable
};
