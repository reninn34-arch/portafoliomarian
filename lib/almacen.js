...
const { variable } = require('./comun');

const RAIZ = path.join(__dirname, '..');
const DATA_FILE = path.join(RAIZ, 'data', 'content.json');
const CLAVE_POR_DEFECTO = process.env.ADMIN_PASS || 'mango2026';

/* Según cómo crees la base en Vercel, la variable se llama POSTGRES_URL,
   DATABASE_URL o con el prefijo STORAGE_. Se aceptan todas. Se descartan las
   variantes de Prisma y las sin pool, que traen parámetros que no nos valen. */
const cadena = () => variable(
  ['POSTGRES_URL', 'DATABASE_URL', 'STORAGE_POSTGRES_URL', 'STORAGE_DATABASE_URL'],
  'DATABASE_URL', /PRISMA|UNPOOLED|NON_POOLING/
) || variable([], 'POSTGRES_URL', /PRISMA|UNPOOLED|NON_POOLING/);
